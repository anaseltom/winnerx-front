    ...
    return (
        <tr>
            <td className="shoping__cart__item">
                <img className="image" src={`${product?.image_url_main}`} style={{objectFit: "contain"}} alt="" />
            ...
            <td className="shoping__cart__quantity">
                <div className="quantity">
                    <div className="pro-qty">
                        <span onClick={()=> {changeQty(product?.product_code,'dec'); }} className="dec qtybtn">-</span>
                        <input type="text" value={prodQty}  disabled />
                        <span onClick={()=> {changeQty(product?.product_code,'inc');}} className="inc qtybtn">+</span>
                    </div>
                </div>
            </td>
            <td className="shoping__cart__total">
                {product?.currency_code} {numberWithCommas(prodQty * product?.unit_price)}
            </td>
            <td className="shoping__cart__item__close">
                <img onClick={ () => {deleteProd(product?.product_code)} } className="social_media" src="/assets/img/delete.png" style={{minWidth: "16px"}} alt="" />
            </td>
        </tr>
    );
};
