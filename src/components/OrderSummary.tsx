...
const CheckoutSummary: React.FC<any> = ({product, qty, setProd, setRefundIsOpen, date, packageStatus, setProdIsOpen}) => {
    ...
    return (
        <tr>
            <td onClick={()=> { setProd(product); setProdIsOpen(true); }} className="shoping__cart__item" style={{width: "40%"}}>
                <img className="image" style={{objectFit: "contain"}} src={`${product?.imageUrlMain}`} alt="" />
            ...
            <td className="shoping__cart__total" style={{width: "15%"}}>
                {
                    packageStatus?.toLowerCase()==="delivered" || packageStatus?.toLowerCase().includes("refunded")  ?  
                    <span className="delivery_status success">{packageStatus?.replace('_', ' ').toUpperCase()}</span>
                    :
                    packageStatus?.toLowerCase()==="pending" || packageStatus?.toLowerCase().includes("warehouse") || packageStatus?.toLowerCase().includes("refund_request") ?
                    <span className="delivery_status pending">{packageStatus?.replace('_', ' ').toUpperCase()}</span>
                    :
                    <span className="delivery_status">{packageStatus?.replace('_', ' ').toUpperCase()}</span>
                }
            </td>
            <td className="shoping__cart__item__close" style={{width: "10%"}}>
                {
                    enableRequest ? 
                    <button onClick={() => { setProd(product); setRefundIsOpen(true); }} type="submit" className="site-btn button-refund">Request Refund</button>
                    :
                    ""
                }
            </td>
